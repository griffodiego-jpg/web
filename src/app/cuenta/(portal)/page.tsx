...
import { LanzamientosCarousel } from "@/components/cuenta/LanzamientosCarousel";
...
export default async function ResumenPage() {
  ...
  // Lanzamientos: SOLO los del tipo "lanzamiento", ordenados por fecha desc.
  // Cap a 12 (= 4 páginas de 3) para que el carrusel sea finito.
  const lanzamientos = (await listNovedades())
    .filter((n) => n.tipo === "lanzamiento")
  ...
  return (
    <div className="max-w-5xl space-y-6">
      ...
      {lanzamientos.length > 0 ? (
        <LanzamientosCarousel items={lanzamientos} />
      ) : null}
    </div>
  );
}
