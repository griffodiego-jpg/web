...
### ✅ Pagos/Recibos en ClientAccountStatus (resuelto 2026-04-22)

El técnico confirmó: los **Recibos vienen con `comp: "RC"`** en la
respuesta de `/ERP/ClientAccountStatus`. Antes el endpoint no los
incluía; ahora sí.

El clasificador `src/lib/b2b/movement-classifier.ts` ya matcheaba
`/^RC/i` como "pago", así que los recibos aparecen automáticamente
en el tab "Pagos / Recibos" + se cuentan en el saldo.

**Pendiente de verificar**: que el campo `haber` de las NCs venga
siempre positivo. En las primeras muestras se vieron NCs con
`haber < 0`, generando "Total haber" negativo. Si sigue pasando, el
técnico debería revisar el signo.
